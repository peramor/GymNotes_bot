...
selectExerciseScene.enter(ctx => {
  ...
  userDb.getLastResults(ctx.from.id, ctx.session.exercise.name)
    .then(res => {
      ...
      ctx.reply(enterMessage, Extra.markup(Markup.keyboard(enterKeyboard)))
    })
})
...
selectExerciseScene.on('message', ctx => {
  ...
  if (repeat.match(regWeightCount)) {
    if (format === 'weight,count') {
      const data = repeat.split(/[-–—]/)
      return ctx.session.exercise.repeats.push({ count: data[0], weight: data[1] })
    }
    else return ctx.reply(errorMessage)
  }

  else if (format === 'time')
    return ctx.session.exercise.repeats.push({ time: ctx.message.text })

  else if (repeat.match(regCount)) {
    if (format === 'count')
      return ctx.session.exercise.repeats.push({ count: repeat })
    else return ctx.reply(errorMessage)
  }
