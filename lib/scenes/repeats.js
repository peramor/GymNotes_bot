...
// handler for user's edited message
repeatsScene.on('edited_message', async ctx => {
  const format = ctx.session.exercise.format
  const editedRepeat = ctx.editedMessage.text
  let repeats = ctx.session.exercise.repeats
  const index = repeats.findIndex(r => r.messageId === ctx.editedMessage.message_id)
  ...
  if (format === 'weight,count' && editedRepeat.match(regWeightCount)) {
    ...
    repeats[index].count = data[1]
    return ctx.reply(`Подход №${index + 1} изменен`)
  }
  else if (format === 'count' && editedRepeat.match(regCount)) {
    repeats[index].count = editedRepeat
    return ctx.reply(`Подход №${index + 1} изменен`)
  }
  ...
  else throw new FormatException(ctx)
})
...
repeatsScene.hears(regWeightCount, ctx => {
  ...
  if (format === 'weight,count') {
    ...
    ctx.session.exercise.repeats.push({
