 ...
 * @param {String?} botReply - text which contains reply of bot
...
async function track(ctx, params = {}) {
  let msg = {
    ...
    intent = params.intent ? params.intent : ctx.scene.current.id
  }
...
function processStart(msg) {
  ...
  if (startMatch[1]) {
    ...
    msg.url = 'opendoor' // if somebody has found bot by self
}
 ...
 */
async function prepareAndSend(msg, botReplyMessage) {
  let data = {
    api_key: token,
    version: version,
    platform: "Telegram",
  ...
  if (botReplyMessage) {
    let botReplyData = JSON.parse(JSON.stringify(data)) // clone object, not linking!
    botReplyData.type = 'agent'
    botReplyData.message = botReplyMessage
    messages.push(botReplyData)
  }
